...
quick synchronization to my computers to be able to pick up the changes there.
...
available options:
 - *Dropbox* ::
   I'm not a customer of Dropbox so this was a no-go to begin with.

 - *WebDAV* ::
   As far as I know I don't have an easily available WebDAV server that I can
   use which makes it easy to get the files to my laptops and desktop systems
   where I can use them with Emacs without doing any fancy scripting to sync
   the files by hand.

 - *Directory* ::
   This just syncs to a directory on the device, doesn't sound like much. But
   it's the option I went for.
