...
memory that has just been turned off due to noise. The obvious solution is
to re-purpose that install and move it to a location where noise doesn't
matter, e.g. the closet (a.k.a the server room) where the file server lives.
...
But to do this move I had a certain list of requirements.
...
I followed these steps of partitioning grabbed from the [[https://nixos.wiki/wiki/NixOS_on_ZFS#Single-disk][NixOS on ZFS]] page
but modified my steps.

#+BEGIN_SRC sh
# Defining some helper variables (these will be used in later
# code blocks as well, so make sure to use the same terminal
# session or redefine them later)
...
#+BEGIN_SRC nix
{
  # You can probably use his one on true, but
  # my UEFI is buggy.
  boot.loader.efi.canTouchEfiVariables = false;

  # This is the regular setup for grub on UEFI which
  # manages /boot automatically.
...
And with the module system NixOS makes it very easy to keep the two ESP
partitions in sync.

I have tested the entire process to make sure it works on my hardware by:
 1. Shut it down
 2. Unplug one drive
 3. Boot it up all the way into the system and watched ZFS be a bit grump
    about some missing disk (oh what a surprise!)
 4. Shut it down
 5. Plug in he first drive and unplug the other one
 6. Repeat step 3 again
 7. Shut i down
 8. Boot with both disks and tell ZFS that I'm sure that there's no issue
    with my hardware and that I don't want to replace my drives.
