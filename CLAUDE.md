...
**Hugo is pinned via nixpkgs** and bundled with `dart-sass` (required for SCSS compilation) using `symlinkJoin`. The theme and all its static dependencies (FontAwesome, flag SVGs, model-viewer) are vendored directly in this repo. The only Nix inputs are `nixpkgs` and `flake-utils`.

**3D models** are consumed as a pre-built GitHub release from `github:etu/3d-models` (no longer a flake input). `_3dmodelsRelease` fetches the tarball via `pkgs.fetchzip` using the GitHub "latest release" permalink — only the pinned hash needs updating when a new release is published. Releases are tagged `YYYY-MM-DD-<short-sha>` in the 3d-models repo.
...
| 3D model assets | fetched at build time | GitHub release from `github:etu/3d-models` — update `_3dmodelsRelease` hash in `flake.nix` |
